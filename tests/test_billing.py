...
class TestBillingEndpointsWithAuth:
    ...
    def test_subscription_free_user(self, mock_validate, client, mock_user_data):
        ...
        response = client.get("/api/v1/billing/subscription", headers={"X-API-Key": "test_key"})
    ...
    def test_checkout_invalid_tier(self, mock_validate, client, mock_user_data):
        ...
        response = client.post(
            "/api/v1/billing/checkout", headers={"X-API-Key": "test_key"}, json={"tier": "invalid_tier"}
        )
    ...
    def test_portal_no_customer(self, mock_validate, client, mock_user_data):
        ...
        mock_validate.return_value = mock_user_data
        response = client.post("/api/v1/billing/portal", headers={"X-API-Key": "test_key"})
        assert response.status_code == 400
    ...
    def test_cancel_no_subscription(self, mock_validate, client, mock_user_data):
        ...
        mock_validate.return_value = mock_user_data
        response = client.post("/api/v1/billing/cancel", headers={"X-API-Key": "test_key"})
        assert response.status_code == 400
...
class TestStripeWebhooks:
    ...
    def test_webhook_missing_signature(self, client):
        ...
        response = client.post("/api/v1/webhooks/stripe", content=b'{"type": "test"}')
        assert response.status_code == 400
    ...
    def test_webhook_invalid_payload(self, mock_construct, client):
        ...
        response = client.post("/api/v1/webhooks/stripe", content=b"invalid", headers={"Stripe-Signature": "test_sig"})
        assert response.status_code == 400
    ...
    def test_webhook_checkout_completed(self, mock_update, mock_get_user, mock_construct, client):
        """Test webhook handles checkout.session.completed"""
        mock_construct.return_value = {
            "type": "checkout.session.completed",
            "data": {
                "object": {"customer": "cus_test123", "subscription": "sub_test123", "metadata": {"tier": "starter"}}
            },
        }
    ...
    def test_webhook_subscription_deleted(self, mock_tier, mock_update, mock_get_user, mock_construct, client):
        ...
        mock_update.assert_called_with(
            "key_hash_123", subscription_id=None, subscription_status="canceled", tier="free"
        )
