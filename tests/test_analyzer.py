...
from src.analyzer.analyzer import AnalysisResult, ContentAnalyzer
...
class TestContentAnalyzer:
    ...
    def test_analyzer_init_defaults(self):
        ...
        assert analyzer.model == "mistral-small-latest"
        assert "mistral" in analyzer.api_url

    def test_analyzer_init_custom(self):
        """Test ContentAnalyzer with custom settings"""
        analyzer = ContentAnalyzer(model="mistral-large-latest")
...
class TestAnalyzerAsync:
    ...
    async def test_analyze_changes_mistral_error(self):
        """Test analysis with Mistral API error"""
        analyzer = ContentAnalyzer()
        ...
        with patch("httpx.AsyncClient") as mock_client:
            ...
            result = await analyzer.analyze_changes(
                url="https://example.com", old_content="old", new_content="new", diff="-old\n+new"
            )
    ...
    async def test_analyze_changes_invalid_json(self):
        ...
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_response.json.return_value = {"choices": [{"message": {"content": "This is not valid JSON at all"}}]}
    ...
    async def test_summarize_content_success(self):
        ...
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_response.json.return_value = {"choices": [{"message": {"content": "Résumé du contenu web."}}]}
