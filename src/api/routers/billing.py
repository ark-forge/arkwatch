...
class CheckoutRequest(BaseModel):
    ...
    trial_days: int | None = 14  # Default 14-day free trial
...
async def create_checkout_session(request: CheckoutRequest, user: dict = Depends(get_current_user)):
    ...
    if not customer_id:
        customer_id = StripeService.create_customer(email=user["email"], name=user["name"], api_key_hash=key_hash)
        update_stripe_info(key_hash, customer_id=customer_id)

    # Create checkout session
    success_url = request.success_url or f"{APP_URL}/checkout-success.html?session_id={{CHECKOUT_SESSION_ID}}"
    cancel_url = request.cancel_url or f"{APP_URL}/checkout-cancel.html"

    try:
        session = StripeService.create_checkout_session(
            customer_id=customer_id, tier=request.tier, success_url=success_url, cancel_url=cancel_url, promotion_code=request.promotion_code, trial_days=request.trial_days
        )
        return session
...
async def create_portal_session(user: dict = Depends(get_current_user)):
    ...
    try:
        session = StripeService.create_billing_portal_session(customer_id=customer_id, return_url=return_url)
        return session
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Stripe error: {str(e)}") from e
